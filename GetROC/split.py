...
FinishList=[]
for f in os.listdir('.'):
   if not 'ROC_2017_output_' in f:
     continue
   FinishList.append(int(f.replace('ROC_2017_output_','').replace('.root','')))

for i in range(m):
  f = open('batch_%s.sh' % str(i),"w+")
  f.writelines("#!/bin/sh\n\n")
  f.writelines("cd /afs/cern.ch/work/z/zhixing/private/CMSSW_7_4_14/src/CMSDIJET/DijetRootTreeAnalyzer/GetROC\n\n")
  f.writelines("eval `scramv1 runtime -sh`\n\n")
  f.writelines("python GetROC.py -i filelist_%s.txt -o /tmp/ROC_2017_output_%s.root\n\n"%(str(i), str(i) ))
  f.writelines("mv /tmp/ROC_2017_output_%s.root"%str(i)+" .\n\n")
  f.close()
