...
for i in Com_list:
   index+=1
   a = open('aloha_Inter_'+nobtag+ns+str(index)+'.sh','w+')
   a.write('#!/bin/bash\n\n'+i)
   a.close()
   os.system('chmod 751 aloha_Inter_'+nobtag+ns+str(index)+'.sh')
a= open('aloha_Inter_'+nobtag+ns+'0.sh','w+')
a.write('#!/bin/bash\n\n'+Total)
a.close()
os.system('chmod 751 aloha_Inter_'+nobtag+ns+'0.sh')
