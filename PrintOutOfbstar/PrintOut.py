...
#    for i in progressbar(range(nEntries), 'Progress: ', 40):
    for i in range(nEntries):
      ...
      if not (abs(tchain.deltaETAjj)<1.3       and
                ...
                tchain.PassJSON):
            continue
      if abs(tchain.jetpflavour_j1) ==  5 and abs(tchain.jetpflavour_j2) ==  21 :
        n1+=1
	continue  

      if abs(tchain.jetpflavour_j1) ==  21 and abs(tchain.jetpflavour_j2) ==  5 :
        n1+=1
	continue

      #print [abs(int(tchain.jetpflavour_j1)),abs(int(tchain.jetpflavour_j2))]
