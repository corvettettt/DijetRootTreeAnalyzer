...
#    for t in range(2000):
    ...
    for k in csv_list: 
      TH_DCSV[k]={}
      TH_CSV[k]={}
      for i in ['1','2','0','le1','all']:
	TH_DCSV[k][i] = TH1D('NoE'+i+str(int(k*10000))+'DCSV','number of Event passing '+i+'DeepCSV when cut='+str(k),12,1200,6000)
	TH_CSV[k][i] = TH1D('NoE'+i+str(int(k*10000))+'CSV','number of Event passing '+i+'CSVv2 when cut='+str(k),12,1200,6000)
      for j in range(len(L)-1):
	for i in ['1','2','0','le1']:
	   TH_DCSV[k][i].SetBinContent(j+1,N_pass_DCSV[j][i][k])
	   TH_CSV[k][i].SetBinContent(j+1,N_pass_CSV[j][i][k])
        TH_DCSV[k]['all'].SetBinContent(j+1,N_all[j]['0'][k])
	TH_CSV[k]['all'].SetBinContent(j+1,N_all[j]['0'][k])


    Fout = TFile(Output,'recreate')
    for i in csv_list:
      for j in ['1','2','0','le1','all']:
	TH_DCSV[i][j].Write()
	TH_CSV[i][j].Write()
