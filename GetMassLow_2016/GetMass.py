...
#    L = [800, 1313, 1383, 1455, 1530, 1607, 1687, 1770, 1856, 1945, 2037, 2132, 2231, 2332, 2438, 2546, 2659, 2775, 2895, 3019, 3147, 3279, 3416, 3558, 3704, 3854, 4010, 4171, 4337, 4509, 4686, 4869, 5058, 5253, 5455, 5663, 5877, 6099, 6328, 6564, 6808, 7060, 7320, 7589, 7866, 6000]
    L = [800,1200,1600,2000,2400,2800,3200,3600,4000,4400,4800,5200,5600,6000]
    ...
    h_CSV_mjj1={}
    h_CSV_mjj2={}
    h_mjj = TH1F('total','total',len(L)-1,800,6000)
    for i in csv_list:
       h_DCSV_mjj0[i] = TH1F('pass_0DeepCSV'+str(i),'pass_0DeepCSV'+str(i),len(L)-1,800,6000)
       h_DCSV_mjj1[i] = TH1F('pass_1DeepCSV'+str(i),'pass_1DeepCSV'+str(i),len(L)-1,800,6000)
       h_DCSV_mjjle1[i]=TH1F('pass_le1DeepCSV'+str(i),'pass_le1DeepCSV'+str(i),len(L)-1,800,6000)
       h_DCSV_mjj2[i] = TH1F('pass_2DeepCSV'+str(i),'pass_2DeepCSV'+str(i),len(L)-1,800,6000)
...
#    h_deltaphi = TH1F('DeltaPhi','DeltaPhi',50,0,3.14)
       h_CSV_mjj0[i] = TH1F('pass_0CSV'+str(i),'pass_0CSV'+str(i),len(L)-1,800,6000)
       h_CSV_mjj1[i] = TH1F('pass_1CSV'+str(i),'pass_1CSV'+str(i),len(L)-1,800,6000)
       h_CSV_mjjle1[i]=TH1F('pass_le1CSV'+str(i),'pass_le1CSV'+str(i),len(L)-1,800,6000)
       h_CSV_mjj2[i] = TH1F('pass_2CSV'+str(i),'pass_2CSV'+str(i),len(L)-1,800,6000)
