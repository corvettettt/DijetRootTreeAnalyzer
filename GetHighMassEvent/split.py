...
for f in os.listdir('.'):
   if not 'output' in f:
     continue
   FinishList.append(int(f.replace('output_','').replace('.txt','')))

for i in range(m):
  f = open('batch_%s.sh' % str(i),"w+")
  f.writelines("#!/bin/sh\n\n")
  f.writelines("cd /afs/cern.ch/work/z/zhixing/private/CMSSW_7_4_14/src/CMSDIJET/DijetRootTreeAnalyzer/GetHighMassEvent/\n\n")
  f.writelines("eval `scramv1 runtime -sh`\n\n")
  f.writelines("python GetMassHighMassEvent.py -i filelist_%s.txt -o /tmp/output_%s.txt\n\n"%(str(i), str(i) ))
  f.writelines("mv /tmp/output_%s.txt"%str(i)+" .\n\n")
  f.close()
...
#  print "bsub -q 8nd -J jobs%s < batch_%s.sh"%(str(i),str(i))
