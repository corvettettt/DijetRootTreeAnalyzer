...
if __name__=='__main__':
  ...
  if model == 'qq':
    ...
    flavor = 'bg'
  for i in CSV_Value:
    print('\n\n\npython python/Working2_2017.py -b '+tag+str(int(i*1000))+' -f '+tag+str(int(i*1000))+'Scan -o cards_'+tag+str(int(i*1000))+'_scan -s '+Folder+'/ResonanceShapes_'+model+'_'+flavor+'_13TeV_Spring16_'+str(int(i*1000))+'_Nominal_Interpolation_rescale.root -m '+model+' -p ' + point+'\n\n\n')
    os.system('python python/Working2_2017.py -b '+tag+str(int(i*1000))+' -f '+tag+str(int(i*1000))+'Scan -o cards_'+tag+str(int(i*1000))+'_scan -s '+Folder+'/ResonanceShapes_'+model+'_'+flavor+'_13TeV_Spring16_'+str(int(i*1000))+'_Nominal_Interpolation_rescale.root -m '+model+' -p ' + point)
