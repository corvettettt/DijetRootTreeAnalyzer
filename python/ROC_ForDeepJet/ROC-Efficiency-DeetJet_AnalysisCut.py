...
if __name__ == '__main__':
  ...
  for sample in SampleName.keys()+['All']:
	...
	ratio[sample][mass][flavor] = {}
	    ...
	    if sample == 'bstar' and flavor == 'udsg' and  discri == 0.8001:
	      print 'mass:',mass,'  Num:',numerator,'  Den:',denominator 
    ...
    for mass in ll:
      ...
      for flavor in ['g','uds','b','c','udsg']:
        ...
        for tagger in ['CSVv2','DeepCSV','DeepJet']:
          ...
          if flavor == 'udsg':
  	    gr_dis[sample][mass][flavor][tagger].SetTitle('%s MisIdentify Probablity of Mass %sGeV %s sample;discriminator;MisIdentify Probablity (%s jet)'%(tagger,mass,sample,flavor))
  	  else:
  ...
  for sample in SampleName.keys()+['All']:
	...
	for WP in ['L','M','T']: 
	  gr_mass[sample][flavor][tagger][WP]= TGraph()
          ll = SampleName['R2bb'].keys() if sample=='All' else SampleName[sample].keys()
	  if flavor == 'udsg':
	    gr_mass[sample][flavor][tagger][WP].SetTitle('%s WP %s MisIdentify Probablity of %s sample;Mass(GeV);MisIdentify Probablity(%s jet)'%(WP,tagger,sample,flavor))
	  if flavor == 'b':
          ...
          for index,mass in enumerate(sorted(ll)):
            gr_mass[sample][flavor][tagger][WP].SetPoint(index,int(mass),ratio[sample][mass][flavor][tagger][WorkingPoint[tagger][WP]])
  ...
  Fout = TFile('ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/NewDeep_ROC_Efficiency.root','recreate')
