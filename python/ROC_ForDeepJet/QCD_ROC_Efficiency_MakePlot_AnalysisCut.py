...
QCD_Sample = {
'200-300':'/eos/cms/store/group/phys_exotica/dijet/Dijet13TeV/TylerW/2017JetHT_reduced/QCD/QCD200_300GeV_reduced_skim.root',
'300-500':'/eos/cms/store/group/phys_exotica/dijet/Dijet13TeV/TylerW/2017JetHT_reduced/QCD/QCD300_500GeV_reduced_skim.root',
'500-700':'/eos/cms/store/group/phys_exotica/dijet/Dijet13TeV/TylerW/2017JetHT_reduced/QCD/QCD500_700GeV_reduced_skim.root',
'700-1000':'/eos/cms/store/group/phys_exotica/dijet/Dijet13TeV/TylerW/2017JetHT_reduced/QCD/QCD700_1000GeV_reduced_skim.root',
'1000-1500':'/eos/cms/store/group/phys_exotica/dijet/Dijet13TeV/TylerW/2017JetHT_reduced/QCD/QCD1000_1500GeV_reduced_skim.root',
'1500-2000':'/eos/cms/store/group/phys_exotica/dijet/Dijet13TeV/TylerW/2017JetHT_reduced/QCD/QCD1500_2000GeV_reduced_skim.root',
'2000-4000':'/eos/cms/store/group/phys_exotica/dijet/Dijet13TeV/TylerW/2017JetHT_reduced/QCD/QCD2000_InfGeV_reduced_skim.root',
}
...
Fin = TFile('ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/NewDeep_ROC_Efficiency.root')
Fin_QCD  = TFile('QCD_ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/NewDeep_ROC_Efficiency.root')
Fout = TFile('QCD_ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/ROC.root','recreate')
...
for index,pT in enumerate(Pt_Range[0:-1]):
  for tagger in ['CSVv2','DeepCSV','DeepJet']:
	...
	den = Fin_QCD.Get('pt_QCD_udsg_'+tagger+'_'+str(pT)).GetY()[CSV_Value[tagger].index(WorkingPoint[tagger][WP])]
	if den<0:
	   den = 0
	if den == 0:
	  ratio = 0
...
for sample in SampleName.keys()+['All']:
  ...
  for index,pT in enumerate(Pt_Range[0:-1]):
    ...
    for tagger in ['CSVv2','DeepCSV','DeepJet']:
       ...
       Yaxis = Fin_QCD.Get('pt_QCD_g_'+tagger+'_'+str(pT)).GetY()
...
for sample in SampleName.keys()+['All']:
  ...
  for ind,pT in enumerate(Pt_Range[0:-1]):
    ...
    for tagger in ['CSVv2','DeepCSV','DeepJet']:
       ...
       Xaxis = Fin.Get('pt_'+sample+'_b_'+tagger+'_'+str(pT)).GetY()
       Yaxis = Fin_QCD.Get('pt_QCD_uds_'+tagger+'_'+str(pT)).GetY()
       for index in range(len(CSV_Value[tagger])):
...
for sample in SampleName.keys()+['All']:
  ROCudsg_pt[sample] = {}
  for ind,pT in enumerate(Pt_Range[0:-1]):
    ROCudsg_pt[sample][pT] = {}
    for tagger in ['CSVv2','DeepCSV','DeepJet']:
       ROCudsg_pt[sample][pT][tagger] = TGraphAsymmErrors()
       ROCudsg_pt[sample][pT][tagger].SetTitle('%s ROC plot of %s sample within pT %d ~ %d;b Tagging Efficiency;MisIdentify Probablity (uds jet)'%(tagger,sample,Pt_Range[ind],Pt_Range[ind+1]))
       Xaxis = Fin.Get('pt_'+sample+'_b_'+tagger+'_'+str(pT)).GetY()
       Yaxis = Fin_QCD.Get('pt_QCD_udsg_'+tagger+'_'+str(pT)).GetY()
       for index in range(len(CSV_Value[tagger])):
         ROCudsg_pt[sample][pT][tagger].SetPoint(index,Xaxis[index],Yaxis[index])
...
for sample in SampleName.keys()+['All']:
  ...
  for ind,pT in enumerate(Pt_Range[0:-1]):
    ...
    for tagger in ['CSVv2','DeepCSV','DeepJet']:
       ...
       Xaxis = Fin.Get('pt_'+sample+'_b_'+tagger+'_'+str(pT)).GetY()
       Yaxis = Fin_QCD.Get('pt_QCD_c_'+tagger+'_'+str(pT)).GetY()
       for index in range(len(CSV_Value[tagger])):
...
WriteEverything(ROCudsg_pt,'ROCudsg_pt')
...
for sample in SampleName.keys()+['All']:
  for tagger in ['CSVv2','DeepCSV','DeepJet']:
    ...
    leg.Draw('same')
    c.Print('QCD_ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/TaggingPower_Mass_%s_%s.pdf'%(tagger,sample))
    c.Close()
...
for sample in SampleName.keys()+['All']:
  for tagger in ['CSVv2','DeepCSV','DeepJet']:
    ...
    leg.Draw('same')
    c.Print('QCD_ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/TaggingPower_pT_%s_%s.pdf'%(tagger,sample))
    c.Close()
  ...
  for flavor in ['b','udsg','c','uds','g']:
    for tagger in ['CSVv2','DeepCSV','DeepJet']:
	  ...
	  if flavor =='b':
	    ...
	    working = Fin_QCD.Get('mass_'+'QCD'+'_'+flavor+'_'+tagger+'_'+WP)
      ...
      leg.Draw('same')
      if flavor=='b':
 	   c.Print('QCD_ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/bEffi_%s_%s_VSmass.pdf'%(tagger,sample))
      else:
	   c.Print('QCD_ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/%sMP_%s_%s_VSmass.pdf'%(flavor,tagger,'QCD'))
      c.Close()
...
for sample in SampleName.keys()+['All']:
  ...
  for mass in ll:
    ...
    leg.Draw('same')
    c.SetLogy()
    c.Print('QCD_ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/ROC_%s_%s.pdf'%(sample,mass))
    c.Close()
...
for sample in SampleName.keys()+['All']:
  for ind,pT in enumerate(Pt_Range[0:-1]):
    ...
    leg.Draw('same')
    c.SetLogy()
    c.Print('QCD_ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/ROC_%s_Pt%d-%d.pdf'%(sample,Pt_Range[ind],Pt_Range[ind+1]))
    c.Close()
...
for sample in SampleName.keys()+['All']:
  ...
  for mass in ll:
    ...
    leg.Draw('same')
    c.SetLogy()
    c.Print('QCD_ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/ROCuds_%s_%s.pdf'%(sample,mass))
    c.Close()
...
for sample in SampleName.keys()+['All']:
  for ind,pT in enumerate(Pt_Range[0:-1]):
    ...
    leg.Draw('same')
    c.SetLogy()
    c.Print('QCD_ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/ROCuds_%s_Pt%d-%d.pdf'%(sample,Pt_Range[ind],Pt_Range[ind+1]))
    c.Close()
...
for sample in SampleName.keys()+['All']:
  ...
  for mass in ll:
    ...
    leg.Draw('same')
    c.SetLogy()
    c.Print('QCD_ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/ROCg_%s_%s.pdf'%(sample,mass))
    c.Close()
...
for sample in SampleName.keys()+['All']:
  for ind,pT in enumerate(Pt_Range[0:-1]):
    ...
    leg.Draw('same')
    c.SetLogy()
    c.Print('QCD_ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/ROCg_%s_Pt%d-%d.pdf'%(sample,Pt_Range[ind],Pt_Range[ind+1]))
    c.Close()
...
for sample in SampleName.keys()+['All']:
  ...
  for mass in ll: 
    ...
    leg.Draw('same')
    c.SetLogy()
    c.Print('QCD_ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/ROCc_%s_%s.pdf'%(sample,mass))
    c.Close()
...
for sample in SampleName.keys()+['All']:
  for ind,pT in enumerate(Pt_Range[0:-1]):
    ...
    leg.Draw('same')
    c.SetLogy()
    c.Print('QCD_ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/ROCc_%s_Pt%d-%d.pdf'%(sample,Pt_Range[ind],Pt_Range[ind+1]))
    c.Close()

for sample in ['All']:
  for index,tagger in enumerate(['CSVv2','DeepCSV','DeepJet']):
    for jet in ['b','uds','udsg','c','g']:
      c = TCanvas()
      c.SetGridx()
      c.SetGridy()
      color = 0
      leg = TLegend(0.7,0.6,0.9,0.89)
      for ind,pT in enumerate(Pt_Range[0:-1]): 
	if not (pT == 30 or pT==400 or pT==800 or pT==1000 or pT ==1400):
	  continue
	color +=1
	if color ==5 :
	  color +=1
	if jet =='b':
          pl = Fin.Get('pt_%s_%s_%s_%s'%(sample,jet,tagger,str(pT)))
	  pl.GetYaxis().SetRangeUser(0.0001,1)
	  pl.SetTitle('%s btagging Efficiency;discriminator;btagging Efficiency'%(tagger))
        else:
	  pl = Fin_QCD.Get('pt_QCD_%s_%s_%s'%(jet,tagger,str(pT)))
	  pl.GetYaxis().SetRangeUser(0.0001,1)
          pl.SetTitle('%s MisIdentify Probability of %s jet;discriminator;MisIdentify Probability(%s jet)'%(sample,jet,jet))
        pl.SetLineColor(color)
        pl.SetMarkerColor(color)
        pl.SetMarkerSize(0.1)
	leg.AddEntry(pl,'pT%s-%s'%(Pt_Range[ind],Pt_Range[ind+1]),'L')
        if ind == 0:
	  pl.Draw()
        else:
	  pl.Draw('same')
...
#      c.SetLogy()
      if 'b'==jet:
	c.Print('QCD_ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/%sbTaggingEfficiecnyVSpT.pdf'%(tagger))
      else:
        c.Print('QCD_ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/%sMisIProbOf%sVSpT.pdf'%(tagger,jet))
