...
if __name__=='__main__':
  ...
  for i in SampleName[model].keys():
	...
	if getattr(tchain,'jetpflavour_'+k) != 21:
	  continue
	histo[i]['h_pT_%s'%k].Fill(getattr(tchain,'pTWJ_'+k))
        histo[i]['h_Eta_%s'%k].Fill(getattr(tchain,'etaWJ_'+k))
        histo[i]['h_Phi_%s'%k].Fill(getattr(tchain,'phiWJ_'+k))
	histo[i]['h_pT'].Fill(getattr(tchain,'pTWJ_'+k))
  ...
  Fout = TFile('L1l2_%s.root'%model,'recreate')
