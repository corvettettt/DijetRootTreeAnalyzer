...
Fin_QCD = TFile('ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/NewDeep_ROC_Efficiency.root')
#Fin_QCD  = TFile('QCD_ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/NewDeep_ROC_Efficiency.root')
Fout = TFile('ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/ROC.root','recreate')
...
for index,pT in enumerate(Pt_Range[0:-1]):
  for tagger in ['CSVv2','DeepCSV','DeepJet']:
	...
	den = Fin_QCD.Get('pt_'+sample+'_udsg_'+tagger+'_'+str(pT)).GetY()[CSV_Value[tagger].index(WorkingPoint[tagger][WP])]
...
for sample in SampleName.keys()+['All']:
  ...
  for index,pT in enumerate(Pt_Range[0:-1]):
    ...
    for tagger in ['CSVv2','DeepCSV','DeepJet']:
       ...
       Yaxis = Fin_QCD.Get('pt_'+sample+'_g_'+tagger+'_'+str(pT)).GetY()
...
for sample in SampleName.keys()+['All']:
  ...
  for ind,pT in enumerate(Pt_Range[0:-1]):
    ...
    for tagger in ['CSVv2','DeepCSV','DeepJet']:
       ...
       Xaxis = Fin.Get('pt_'+sample+'_b_'+tagger+'_'+str(pT)).GetY()
       Yaxis = Fin_QCD.Get('pt_'+sample+'_uds_'+tagger+'_'+str(pT)).GetY()
       for index in range(len(CSV_Value[tagger])):
...
for sample in SampleName.keys()+['All']:
  ...
  for ind,pT in enumerate(Pt_Range[0:-1]):
    ...
    for tagger in ['CSVv2','DeepCSV','DeepJet']:
       ...
       Xaxis = Fin.Get('pt_'+sample+'_b_'+tagger+'_'+str(pT)).GetY()
       Yaxis = Fin_QCD.Get('pt_'+sample+'_c_'+tagger+'_'+str(pT)).GetY()
       for index in range(len(CSV_Value[tagger])):
...
for sample in SampleName.keys()+['All']:
  ...
  for ind,pT in enumerate(Pt_Range[0:-1]): 
    ...
    for tagger in ['CSVv2','DeepCSV','DeepJet']: 
       ...
       Xaxis = Fin.Get('pt_'+sample+'_b_'+tagger+'_'+str(pT)).GetY()
       Yaxis = Fin_QCD.Get('pt_'+sample+'_udsg_'+tagger+'_'+str(pT)).GetY()
       for index in range(len(CSV_Value[tagger])):
...
for sample in SampleName.keys()+['All']:
  for tagger in ['CSVv2','DeepCSV','DeepJet']:
    ...
    leg.Draw('same')
    c.Print('ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/TaggingPower_Mass_%s_%s.pdf'%(tagger,sample))
    c.Close()
...
for sample in SampleName.keys()+['All']:
  for tagger in ['CSVv2','DeepCSV','DeepJet']:
    ...
    leg.Draw('same')
    c.Print('ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/TaggingPower_pT_%s_%s.pdf'%(tagger,sample))
    c.Close()
...
for sample in SampleName.keys()+['All']:
  for flavor in ['b','udsg','c','uds','g']:
    for tagger in ['CSVv2','DeepCSV','DeepJet']:
      ...
      leg.Draw('same')
      if flavor=='b':
 	   c.Print('ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/bEffi_%s_%s_VSmass.pdf'%(tagger,sample))
      else:
	   c.Print('ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/%sMP_%s_%s_VSmass.pdf'%(flavor,tagger,sample))
      c.Close()
...
for sample in SampleName.keys()+['All']:
  ...
  for mass in ll:
    ...
    leg.Draw('same')
    c.SetLogy()
    c.Print('ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/ROC_%s_%s.pdf'%(sample,mass))
    c.Close()
...
for sample in SampleName.keys()+['All']:
  for ind,pT in enumerate(Pt_Range[0:-1]):
    ...
    leg.Draw('same')
    c.SetLogy()
    c.Print('ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/ROC_%s_Pt%d-%d.pdf'%(sample,Pt_Range[ind],Pt_Range[ind+1]))
    c.Close()
...
for sample in SampleName.keys()+['All']:
  ...
  for mass in ll:
    ...
    leg.Draw('same')
    c.SetLogy()
    c.Print('ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/ROCuds_%s_%s.pdf'%(sample,mass))
    c.Close()
...
for sample in SampleName.keys()+['All']:
  for ind,pT in enumerate(Pt_Range[0:-1]):
    ...
    leg.Draw('same')
    c.SetLogy()
    c.Print('ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/ROCuds_%s_Pt%d-%d.pdf'%(sample,Pt_Range[ind],Pt_Range[ind+1]))
    c.Close()
...
for sample in SampleName.keys()+['All']:
  ...
  for mass in ll:
    ...
    leg.Draw('same')
    c.SetLogy()
    c.Print('ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/ROCg_%s_%s.pdf'%(sample,mass))
    c.Close()
...
for sample in SampleName.keys()+['All']:
  for ind,pT in enumerate(Pt_Range[0:-1]):
    ...
    leg.Draw('same')
    c.SetLogy()
    c.Print('ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/ROCg_%s_Pt%d-%d.pdf'%(sample,Pt_Range[ind],Pt_Range[ind+1]))
    c.Close()
...
for sample in SampleName.keys()+['All']:
  ...
  for mass in ll: 
    ...
    leg.Draw('same')
    c.SetLogy()
    c.Print('ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/ROCc_%s_%s.pdf'%(sample,mass))
    c.Close()
...
for sample in SampleName.keys()+['All']:
  for ind,pT in enumerate(Pt_Range[0:-1]):
    ...
    leg.Draw('same')
    c.SetLogy()
    c.Print('ROC_Efficiency_DeepJet_DeepCSV_CSVv2_AnalysisCut/ROCc_%s_Pt%d-%d.pdf'%(sample,Pt_Range[ind],Pt_Range[ind+1]))
    c.Close()
