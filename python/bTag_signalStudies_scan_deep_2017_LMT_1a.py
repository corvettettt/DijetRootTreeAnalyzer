...
def bookAndFill(mass,sample,flavour):
	...
	for i,j in CSV_Value.items():
	   ...
	   nbtag = 0
	     ...
	     P_DATA_1 =  GetEfficiency(tchain.pTWJ_j1,tchain.etaWJ_j1,i,abs(tchain.jetpflavour_j1)) * getattr(tchain,'DeepSF_%s_j1'%i.lower())
	   else:
	     P_MC_1 = 1 - GetEfficiency(tchain.pTWJ_j1,tchain.etaWJ_j1,i,abs(tchain.jetpflavour_j1))
	     P_DATA_1 = 1 - GetEfficiency(tchain.pTWJ_j1,tchain.etaWJ_j1,i,abs(tchain.jetpflavour_j1)) * getattr(tchain,'DeepSF_%s_j1'%i.lower())
#             SFs.append(getattr(tchain,'DeepSF_%s_j1'%i.lower()))
	     ...
	     P_DATA_2 = GetEfficiency(tchain.pTWJ_j2,tchain.etaWJ_j2,i,abs(tchain.jetpflavour_j2)) * getattr(tchain,'DeepSF_%s_j2'%i.lower())
	   else: 
	     P_MC_2 = 1 - GetEfficiency(tchain.pTWJ_j2,tchain.etaWJ_j2,i,abs(tchain.jetpflavour_j2))
	     P_DATA_2 = 1 - GetEfficiency(tchain.pTWJ_j2,tchain.etaWJ_j2,i,abs(tchain.jetpflavour_j2)) * getattr(tchain,'DeepSF_%s_j2'%i.lower())
...
#    CSV_Value = [0.05,0.1,0.1522,0.2,0.25,0.3,0.35,0.4,0.45,0.4941,0.5803,0.6,0.65,0.7,0.75,0.8,0.85,0.8838,0.9693]
    ...
    outFolder = "signalHistos_"+flavour+'_Jan_For2017Scan_deep_1a'
