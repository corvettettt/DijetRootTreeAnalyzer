...
def bookAndFill(mass,sample,flavour,SFfile):
	...
	for i in CSV_Value:
           ...
           for jet in ['j1','j2']:
	    if getattr(tchain,'jetDeepCSVAK4_%s'%jet) > i:
             if abs(getattr(tchain,'jetpflavour_%s'%jet)) == 5:
               SFs.append(TGraph2DEval(SFfile.Get('SFDeepCSVBdis%s'%(str(int(i*1000)))),getattr(tchain,'pTAK4_%s'%jet),getattr(tchain,'etaAK4_%s'%jet)))
             elif abs(getattr(tchain,'jetpflavour_%s'%jet)) == 4:
               SFs.append(TGraph2DEval(SFfile.Get('SFDeepCSVCdis%s'%(str(int(i*1000)))),getattr(tchain,'pTAK4_%s'%jet),getattr(tchain,'etaAK4_%s'%jet)))
             else:
               SFs.append(0)

	   #print 
	   #print abs(getattr(tchain,'jetpflavour_j1')),':',abs(getattr(tchain,'jetpflavour_j2'))
	   #print SFs
...
#    CSV_Value = [0.05,0.1,0.1522,0.2,0.25,0.3,0.35,0.4,0.45,0.4941,0.5803,0.6,0.65,0.7,0.75,0.8,0.85,0.8838,0.9693]
    ...
    SF_file = rt.TFile('/afs/cern.ch/work/z/zhixing/private/CMSSW_7_4_14/src/CMSDIJET/DijetRootTreeAnalyzer/SF_Interpolation_File/Mapping_%s_Interpolation.root'%su)
    ...
    outFolder = "signalHistos_"+flavour+'_MarInter_For2017Scan_DeepCSV_'+su
