...
#    CSV_Value = [0.1,0.15,0.2219,0.3,0.35,0.4,0.45,0.5, 0.5426,0.6, 0.6324,0.7,0.75,0.8, 0.8484, 0.8958, 0.9535]
  ...
  if model == 'qq':
    ...
    flavor = 'bg'
  for i in CSV_Value:
    print('\n\n\npython python/Working2_2016.py -b '+tag+i+' -f '+tag+i+'Scan -o cards_'+tag+i+'_scan_ns -s '+Folder+'/ResonanceShapes_'+model+'_'+flavor+'_13TeV_Spring16_'+i+'_Nominal_Interpolation_rescale.root -m '+model+' -p ' + point+'\n\n\n')
    os.system('python python/Working2_2016.py -b '+tag+i+' -f '+tag+i+'Scan -o cards_'+tag+i+'_scan_ns -s '+Folder+'/ResonanceShapes_'+model+'_'+flavor+'_13TeV_Spring16_'+i+'_Nominal_Interpolation_rescale.root -m '+model+' -p ' + point)
