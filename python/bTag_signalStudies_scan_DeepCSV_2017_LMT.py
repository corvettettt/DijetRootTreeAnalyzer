...
def bookAndFill(mass,sample,flavour):
	...
	SFs = []
	if (tchain.jetDeepCSVAK4_j1 > CSV_Value['T'] and tchain.jetDeepCSVAK4_j2 > CSV_Value['T']):
            SFs.append(tchain.DeepSF_t_j1)
            SFs.append(tchain.DeepSF_t_j2)
        elif (tchain.jetDeepCSVAK4_j1 > CSV_Value['T'] and tchain.jetDeepCSVAK4_j2 > CSV_Value['M']):
	    SFs.append(tchain.DeepSF_t_j1)
	    SFs.append(tchain.DeepSF_m_j2)
        elif (tchain.jetDeepCSVAK4_j1 > CSV_Value['M'] and tchain.jetDeepCSVAK4_j2 > CSV_Value['T']):
	    SFs.append(tchain.DeepSF_m_j1)
            SFs.append(tchain.DeepSF_t_j2)

	if ((tchain.jetDeepCSVAK4_j1 > CSV_Value['M'] and tchain.jetDeepCSVAK4_j2 > CSV_Value['T']) or (tchain.jetDeepCSVAK4_j1 > CSV_Value['T'] and tchain.jetDeepCSVAK4_j2 > CSV_Value['M'])):
	...
	for i,j in CSV_Value.items():
	   ...
	   #  ss += str(tt)+'\t'
	   #print j
	   #print tchain.jetDeepCSVAK4_j1,'\t',tchain.jetDeepCSVAK4_j2
	   #print getattr(tchain,'DeepSF_%s_j1'%i.lower()),'\t',getattr(tchain,'DeepSF_%s_j2'%i.lower())	
	   #print ss
...
#    CSV_Value = [0.05,0.1,0.1522,0.2,0.25,0.3,0.35,0.4,0.45,0.4941,0.5803,0.6,0.65,0.7,0.75,0.8,0.85,0.8838,0.9693]
    ...
    outFolder = "signalHistos_"+flavour+'_Mar_For2017Scan_DeepCSV_'+su
...
#        g_2btag_rate_Q.Write("g_2btag_rate")
    ...
    for i,j in CSV_Value.items():
       ...
       g_1btag_rate_Q.SetTitle("Tagging Rate;Resonance Mass [GeV];Tagging Rate") 
