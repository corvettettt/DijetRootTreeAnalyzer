...
if __name__=='__main__':
  ...
  os.system('mkdir fits_2018_09')
  os.system('mkdir fits_2018_09/'+fit_dir)
  os.system('mkdir '+out_dir)

  content += 'python python/BinnedFit.py -c config/dijet.config -l 36000 -m '+model+' -s '+signal+' inputs/JetHT_run2016_red_cert_scan.root -b '+box+' -d fits_2018_09/'+fit_dir+' --fit-spectrum\n\n'
  content += 'python python/RunCombine_I_btag.py -m '+model+' -d '+out_dir+' --mass range\(1600,7000,100\) -c config/dijet.config -i fits_2018_09/'+fit_dir+'/DijetFitResults_'+box+'.root -b '+box+' --rMax 20 --xsec 1e-3 -l 36.000 --no-signal-sys\n\n'
  ...
  os.system('python python/BinnedFit.py -c config/dijet.config -l 36000 -m '+model+' -s '+signal+' inputs/JetHT_run2016_red_cert_scan.root -b '+box+' -d fits_2018_09/'+fit_dir+' --fit-spectrum')
  os.system('python python/RunCombine_I_btag.py -m '+model+' -d '+out_dir+' --mass range\(1600,7000,100\) -c config/dijet.config -i fits_2018_09/'+fit_dir+'/DijetFitResults_'+box+'.root -b '+box+' --rMax 20 --xsec 1e-3 -l 36.000 --no-signal-sys')
