...
#    for k in progressbar(range(20), "Mass "+str(mass)+": ", 40):
        tchain.GetEntry(k)
	...
	for i in CSV_Value:
	   ...
	   for jet in ['j1','j2']:
	    if getattr(tchain,'jetCSVAK4_%s'%jet) > i:
  	     if abs(getattr(tchain,'jetpflavour_%s'%jet)) == 5:
	       SFs.append(TGraph2DEval(SFfile.Get('SFCSVv2Bdis%s'%(str(int(i*1000)))),getattr(tchain,'pTAK4_%s'%jet),getattr(tchain,'etaAK4_%s'%jet)))
	     elif abs(getattr(tchain,'jetpflavour_%s'%jet)) == 4:
	       SFs.append(TGraph2DEval(SFfile.Get('SFCSVv2Cdis%s'%(str(int(i*1000)))),getattr(tchain,'pTAK4_%s'%jet),getattr(tchain,'etaAK4_%s'%jet)))
	     else:
	       SFs.append(0)
...
#    CSV_Value = [0.05,0.1,0.1522,0.2,0.25,0.3,0.35,0.4,0.45,0.4941,0.5803,0.6,0.65,0.7,0.75,0.8,0.85,0.8838,0.9693]
    ...
    outFolder = "signalHistos_"+flavour+'_MarInter_For2017Scan_CSVv2_'+su
