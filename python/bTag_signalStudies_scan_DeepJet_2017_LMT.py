...
def bookAndFill(mass,sample,flavour):
    ...
    for k in progressbar(range(nEntries), "Mass "+str(mass)+": ", 40):
        ...
        #implement analysis
	SFs = []
	if (tchain.jetDeepJetAK4_j1 > CSV_Value['T'] and tchain.jetDeepJetAK4_j2 > CSV_Value['T']):
            SFs.append(tchain.DeepJetSF_t_j1)
            SFs.append(tchain.DeepJetSF_t_j2)
        elif (tchain.jetDeepJetAK4_j1 > CSV_Value['T'] and tchain.jetDeepJetAK4_j2 > CSV_Value['M']):
	    SFs.append(tchain.DeepJetSF_t_j1)
	    SFs.append(tchain.DeepJetSF_m_j2)
        elif (tchain.jetDeepJetAK4_j1 > CSV_Value['M'] and tchain.jetDeepJetAK4_j2 > CSV_Value['T']):
	    SFs.append(tchain.DeepJetSF_m_j1)
            SFs.append(tchain.DeepJetSF_t_j2)

	if ((tchain.jetDeepJetAK4_j1 > CSV_Value['M'] and tchain.jetDeepJetAK4_j2 > CSV_Value['T']) or (tchain.jetDeepJetAK4_j1 > CSV_Value['T'] and tchain.jetDeepJetAK4_j2 > CSV_Value['M'])):
	...
	for i,j in CSV_Value.items():
	   ...
	   SFs = []
 	   if tchain.jetDeepJetAK4_j1>j:
             SFs.append(getattr(tchain,'DeepJetSF_%s_j1'%i.lower()))
           if tchain.jetDeepJetAK4_j2>j:
             SFs.append(getattr(tchain,'DeepJetSF_%s_j2'%i.lower()))
	   ...
	   #  ss += str(tt)+'\t'
	   #print j
	   #print tchain.jetDeepJetAK4_j1,'\t',tchain.jetDeepJetAK4_j2
	   #print getattr(tchain,'DeepJetSF_%s_j1'%i.lower()),'\t',getattr(tchain,'DeepJetSF_%s_j2'%i.lower())	
	   #print ss
...
#    CSV_Value = [0.05,0.1,0.1522,0.2,0.25,0.3,0.35,0.4,0.45,0.4941,0.5803,0.6,0.65,0.7,0.75,0.8,0.85,0.8838,0.9693]
    ...
    outFolder = "signalHistos_"+flavour+'_Mar_For2017Scan_DeepJet_'+su
    ...
    for mass,hDict in sorted(mDict.iteritems()):
      for i,j in CSV_Value.items(): 
        ...
        g_an_acc[i].SetPoint(bin,mass,num/den)  #wrong. the reduced ntuples have already the selection implemented
...
#        g_2btag_rate_Q.Write("g_2btag_rate")
    ...
    for i,j in CSV_Value.items():
       ...
       g_mtbtag_rate_Q.SetTitle("Tagging Rate;Resonance Mass [GeV];Tagging Rate") 
       g_mtbtag_rate_Q.Draw("APL")
       g_mtbtag_rate_Q.GetYaxis().SetRangeUser(0,1)
