...
#    CSV_Value=[0.5,0.6,0.7,0.8,0.81, 0.82, 0.83, 0.84, 0.85, 0.86, 0.87, 0.88, 0.89, 0.90, 0.91, 0.92, 0.93, 0.94,0.95,0.96]
    ...
    for i,j in CSV_Value.items():
      ...
      hDict[i]["h_mass_passed"] = rt.TH1F(prefix+"_mass_passed_CSVv2",prefix+"_mass_passed_CSVv2",massRange[mass][0],massRange[mass][1],massRange[mass][2])
      ...
      hDict[i]["h_mass_passed_b"] = rt.TH1F(prefix+"_mass_passed_CSVv2_b",prefix+"_mass_passed_CSVv2_b",massRange[mass][0],massRange[mass][1],massRange[mass][2])
      hDict[i]["h_mass_passed_b"].SetMarkerSize(0.5)
 
      hDict[i]["h_mass_passed_0b"] = rt.TH1F(prefix+"_mass_passed_CSVv2_0b",prefix+"_mass_passed_CSVv2_0b",massRange[mass][0],massRange[mass][1],massRange[mass][2])
      hDict[i]["h_mass_passed_0b"].SetMarkerSize(0.5)
 
      hDict[i]["h_mass_passed_1b"] = rt.TH1F(prefix+"_mass_passed_CSVv2_1b",prefix+"_mass_passed_CSVv2_1b",massRange[mass][0],massRange[mass][1],massRange[mass][2])
      ...
      hDict[i]["h_mass_passed_2b"] = rt.TH1F(prefix+"_mass_passed_CSVv2_2b",prefix+"_mass_passed_CSVv2_2b",massRange[mass][0],massRange[mass][1],massRange[mass][2])
      ...
      hDict[i]["h_mass_passed_le1b"] = rt.TH1F(prefix+"_mass_passed_CSVv2_le1b",prefix+"_mass_passed_CSVv2_le1b",massRange[mass][0],massRange[mass][1],massRange[mass][2])
	...
	for i,j in CSV_Value.items():
	   ...
	   if i =='L' :
	     ...
	     weight = tchain.evtNewBweight_t
           ...
           hDict[i]["h_mass_passed_b"].Fill(tchain.mjj,weight)
           #hDict[i]["h_weight_b"].Fill(weight)
           if NBjet == 0:
...
#    CSV_Value = [0.05,0.1,0.1522,0.2,0.25,0.3,0.35,0.4,0.45,0.4941,0.5803,0.6,0.65,0.7,0.75,0.8,0.85,0.8838,0.9693]
    ...
    outFolder = "signalHistos_"+flavour+'_Dec_ForScan_CSVv2_central_sepe'
    ...
    for i,j in CSV_Value.items():
      ...
      g_btag_rate[i] = rt.TGraphAsymmErrors()
      g_btag_rate[i].SetTitle("g_btag_rate;Resonance Mass [GeV];Tagging Rate")
      g_btag_rate[i].SetLineWidth(2)
      g_0btag_rate[i] = rt.TGraphAsymmErrors()
      g_0btag_rate[i].SetMarkerColor(6)
      g_0btag_rate[i].SetLineColor(6)
      ...
      g_btag_weight[i] = rt.TGraphAsymmErrors()
    ...
    for mass,hDict in sorted(mDict.iteritems()):
      for i,j in CSV_Value.items(): 
        ...
        num = hDict[i]["h_mass_passed_b"].GetSumOfWeights()

        num = hDict[i]["h_mass_passed_b"].GetSumOfWeights()
        g_btag_rate[i].SetPoint(bin,mass,num/den)
    ...
    for i,j in CSV_Value.items(): 
      ...
      for mass,hDict in sorted(mDict.iteritems()):
        ...
        hDict[i]["h_mass_passed_b"].DrawNormalized("sames")
        ...
        leg.AddEntry(hDict[i]["h_mass_passed_b"],"weight","P")
        leg.AddEntry(hDict[i]["h_mass_passed_1b"],"1-tag","P")
        leg.AddEntry(hDict[i]["h_mass_passed_2b"],"2-tag","P")
        leg.AddEntry(hDict[i]["h_mass_passed_le1b"],"ge-tag","P")
        leg.Draw("same")
      ...
      g_btag_rate_Q=Do_Inter(g_btag_rate[i])
...
#        g_0btag_rate_Q.Write("g_0btag_rate")
      g_btag_rate_Q.SetLineWidth(2)
#        g_1btag_rate_Q.Write("g_1btag_rate")
      g_0btag_rate_Q.SetMarkerColor(6)
      g_0btag_rate_Q.SetLineColor(6)
...
#        g_2btag_rate_Q.Write("g_2btag_rate")
      ...
      g_btag_rate_Q.Write("g_weight_btag_rate")
      g_0btag_rate_Q.Write("g_0btag_rate")
      g_1btag_rate_Q.Write("g_1btag_rate")
      g_2btag_rate_Q.Write("g_2btag_rate")
    ...
    for i,j in CSV_Value.items():
       ...
       g_btag_rate_Q.Draw("APL")
       g_btag_rate_Q.GetYaxis().SetRangeUser(0,1)
       g_0btag_rate_Q.Draw("PL,sames")
       g_1btag_rate_Q.Draw("PL,sames")
       g_2btag_rate_Q.Draw("PL,sames")
       g_le1btag_rate_Q.Draw("PL,sames")
   
       leg = rt.TLegend(0.87, 0.80, 0.96, 0.89)
       leg.AddEntry(g_btag_rate_Q,"weight","L")
