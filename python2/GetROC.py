...
#    for t in range(2000):
	...
	Ls_a = array.array('d',Ls) 
        ...
        gr.SetTitle('rate of '+j+' CSVv2 bin mass range '+str(L[i])+'-'+str(L[i+3]) )
        gr.GetXaxis().SetTitle('CSVv2')
        gr.GetYaxis().SetTitle('Efficiency')
        gr.SetName('CSVv2RateOf'+j+'Mass'+str(L[i])+'-'+str(L[i+3]))
	gr.Write()
        ...
        gr1.SetTitle('rate of '+j+' DeepCSV bin mass range '+str(L[i])+'-'+str(L[i+3]) )
        gr1.GetXaxis().SetTitle('DeepCSV')
        gr1.GetYaxis().SetTitle('Efficiency')
        gr1.SetName('DeepCSVRateOf'+j+'Mass'+str(L[i])+'-'+str(L[i+3]))
        gr1.Write()
    Fout.Close()
#    for i in csv_list:
