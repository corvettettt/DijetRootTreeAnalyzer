...
L = [1246, 1313, 1383, 1455, 1530, 1607, 1687, 1770, 1856, 1945, 2037, 2132, 2231, 2332,2500,2700,3000,3300,3600,3900,4400,4800,5200,5600,6000,6500,7000,7500,8000]
...
for i in csv_list:
  Signal = 'signalHistos_bb_'+str(int(i*1000))+'.root' 
  S = TFile(Signal)
  ...
  gr['le1'][i].SetTitle(' DeepCSV tagging power in 4 Catagories for 2016 RSG')
  gr['le1'][i].GetYaxis().SetTitle('S/sqrt(B)')
  gr['le1'][i].GetXaxis().SetTitle('Mass(GeV)')
  gr['le1'][i].Draw('APL')
...
for i in ['1','le1','2']:
  ...
  for j in csv_list:
     ...
     if index == 1:
       ...
       gr[i][j].SetTitle('Several DeepCSV bTag Point of '+i+'b Catagory of 2016 RSG')
