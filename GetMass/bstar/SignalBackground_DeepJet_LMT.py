...
for j,i in csv_list.items():
  ...
  rate_le1 = S.Get('DeepJetle1b_TagRate_'+str(int(float(i)*10000)))
  rate_1 = S.Get('DeepJet1b_TagRate_'+str(int(float(i)*10000)))
  rate_2 = S.Get('DeepJet2b_TagRate_'+str(int(float(i)*10000)))
  rate_mt = S.Get('DeepJetmt_TagRate')
  data_rate_1 = B.Get('DeepJet1b_TagRate_'+str(int(float(i)*10000)))
  data_rate_le1 = B.Get('DeepJetle1b_TagRate_'+str(int(float(i)*10000)))
  data_rate_2 = B.Get('DeepJet2b_TagRate_'+str(int(float(i)*10000)))
  data_rate_mt= B.Get('DeepJetmt_TagRate')
  ...
  for l in range(len(L)-1):
     ...
     print j,'mass:',mass,'nu',rate_1.Eval(mass),'de:',data_rate_1.Eval(mass)
  ...
  gr['le1'][i].SetTitle(';Resonance Mass(GeV);Tagging Power')
  gr['le1'][i].Draw('same')
  ...
  c1.Print('DeepJet/RateOf'+str(int(i*1000))+'.pdf')
...
for i in ['1','le1','2']:
  ...
  for k,j in csv_list.items():
     ...
     if index == 1:
       ...
       gr[i][j].SetTitle(';Resonance Mass(GeV);Tagging Power')
     else : 
       gr[i][j].Draw('PL,sames')
     leg.AddEntry(gr[i][j],'TagOf'+str(int(j*1000)),'L')
  leg.Draw('same')
  c1.Print('DeepJet/RateOf'+i+'bCategory.pdf')
  c1 = 0
