...
L = [800,1200,1600,2000,2400,2800,3200,3600,4000,4400,4800,5200,5600,6000]#[1530, 1607, 1687, 1770, 1856, 1945, 2037, 2132, 2231, 2332,2500,2700,3000,3300,3600,3900,4400,4800,5200,5600,6000,6500,7000,7500,8000]
...
#  	y[j][k][m].append(SumBins(h_mjj[j][k][m],L[i],L[i+1])/SumBins(All,L[i],L[i+1]))
	...
	if not k=='0':
	  if h_mjj[j][k][m].GetBinContent(i)/All.GetBinContent(i) > HighestPoint[m][j]:
	    HighestPoint[m][j] = h_mjj[j][k][m].GetBinContent(i)/All.GetBinContent(i)
