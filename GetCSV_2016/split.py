...
for i in range(m):
  ...
  f.writelines("#!/bin/sh\n\n")
  f.writelines("cd /afs/cern.ch/work/z/zhixing/private/CMSSW_7_4_14/src/CMSDIJET/DijetRootTreeAnalyzer/GetCSV_2016/\n\n")
  f.writelines("eval `scramv1 runtime -sh`\n\n")
  f.writelines("python GetCSV.py -i filelist_%s.txt -o /tmp/outputt_%s.root\n\n"%(str(i), str(i) ))
  f.writelines("mv /tmp/outputt_%s.root"%str(i)+" .\n\n")
  f.close()
