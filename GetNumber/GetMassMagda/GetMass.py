...
if '__main__'==__name__:
    ...
    h_CSV_mjj1={}
    h_CSV_mjj2={}
    h_mjj3 = TH1F('Mjj_1530','Mjj_1530',len(L2)-1, array.array('f',L2))
    h_mjj2 = TH1F('Mjj','Mjj',len(L)-1, array.array('f',L))
    h_mjj = TH1F('total','total',10000,0,10000)
    h_eta = TH1F('eta','eta',100,-3.14,3.14)
    h_delta = TH1F('DeltaEta','DeltaEta',40,0,1.5)
    h_pt1  = TH1F('LeadingPt','LeadingPt',100,0,7000)
    h_pt2  = TH1F('SLeadingPt','SLeadingPt',100,0,7000)
...
#'/eos/cms/store/group/phys_exotica/dijet/Dijet13TeV/juska/moriond17_red_cert_v2/moriond17_v2_37fb_1nd_s15_20170127_164251/'+i.replace('\n','').replace('\t',''))
    ...
    content = '#RUN\t #EVENT\t ETA1\t ETA2\t DeltaETA\t PT1\t PT2\t MJJ\t lumi \n'
         ...
         #       tchain.pTWJ_j2<6500              and
          ...
          #      tchain.mjj > 1246                and
           #     tchain.mjj < 8152               and
                ...
                tchain.PassJSON):
            continue
	h_pt1.Fill(tchain.pTWJ_j1)
        h_pt2.Fill(tchain.pTWJ_j2)
	h_delta.Fill(abs(tchain.deltaETAjj))
	h_eta.Fill(tchain.etaWJ_j1)
        h_eta.Fill(tchain.etaWJ_j2)
        h_mjj.Fill(tchain.mjj)
        h_mjj2.Fill(tchain.mjj)
	if tchain.mjj>1530:
          h_mjj3.Fill(tchain.mjj)
        if tchain.mjj>6000:
          content += str(tchain.run)+'\t '+str(tchain.event)+'\t '+str(tchain.etaWJ_j1)+'\t '+str(tchain.etaWJ_j2)+'\t '+str(tchain.deltaETAjj)+'\t '+str(tchain.pTWJ_j1)+'\t ' +str(tchain.pTWJ_j2)+'\t '+str(tchain.mjj)+'\t '+str(tchain.lumi)+'\n'
    ftxt = open(Output.replace('root','txt'),'w')
    ftxt.write(content)
    ftxt.close()
    h_delta.Write()
    h_eta.Write()
